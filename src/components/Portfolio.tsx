          ...
          <ul className="grid grid-cols-2 gap-3 md:grid-cols-4">
            {languages.map((l) => (
              <li key={l.name} className="rounded-md border border-border bg-card p-3">
                <p className={`font-serif font-medium ${l.name === "russian" ? "text-[10px]" : "text-sm"}`}>{l.name}</p>
                <p className={`text-muted-foreground ${l.name === "russian" ? "text-[8px]" : "text-xs"}`}>{l.level}</p>
              </li>
            ))}
          </ul>
        </Section>
